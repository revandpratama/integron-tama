...
export const getColumnsBase = (lang: 'en' | 'id') => {
    ...
    return [
        ...
        { id: 'DONE', title: t.done, color: '#334155', description: t.doneDesc },
    ] as { id: BoardStage; title: string; color: string; description: string }[];
};
`;

    content = content.replace(
        /\/\/ We'll calculate dynamic backgrounds inside the component to respect light\/dark mode([\s\S]*?)\];/,
        contextStr
    );

    content = content.replace(/COLUMNS_BASE\.forEach/g, 'getColumnsBase(lang).forEach');
    content = content.replace(/COLUMNS_BASE\.some/g, 'getColumnsBase(lang).some');
    content = content.replace(/COLUMNS_BASE\.map/g, 'getColumnsBase(lang).map');
...
function KanbanContent({ lang, setLang }: { lang: 'en' | 'id', setLang: (l: 'en' | 'id') => void }) {
    ...
    // Replace final brace for KanbanPage (which is now KanbanContent)
    // Actually simpler to just not do regex for the curly brace if I don't know where it ends.
    // Instead, just append `}` at the very very very end of the KanbanContent if it's the last export?
    // Wait, PartnerDetailDialog is right after.
    content = content.replace(
        /function PartnerDetailDialog/,
        `} // End of KanbanContent\n\nfunction PartnerDetailDialog`
    );

    content = content.replace(/<Typography variant="h4"(.*?)>Integration Board<\/Typography>/g, '<Typography variant="h4"$1>{t.boardTitle}</Typography>');
    ...
    content = content.replace(/const COLUMNS = useMemo\(\(\) => COLUMNS_BASE/g, 'const COLUMNS = useMemo(() => getColumnsBase(lang)');
    content = content.replace(/\[mode\]\);/g, '[mode, lang]);');

    content = content.replace(/<WarningAmberIcon \/>\s*Incomplete Tasks Detected/, '<WarningAmberIcon />\n                    {t.incompleteTasks}');
    // Check moving text
    content = content.replace(/You are moving this card to <strong>\{COLUMNS_BASE\.find(.*?)\}\?.title\}<\/strong>/g, '{t.movingPre} <strong>{getColumnsBase(lang).find$1}?.title}</strong>');
    ...
    content = content.replace(/COLUMNS_BASE\.find/g, 'getColumnsBase(lang).find');
    ...
    // In KanbanCard SLA Overdue text needs fixes because it's wrapped in template literals.
    ...
    content = content.replace(/`SLA Overdue by \$\{Math\.abs\(slaRemaining\)\} day\(s\)`/g, '`${t.slaOverdue} ${Math.abs(slaRemaining)} ${t.days}`');
    ...
    console.log("Rewrote page.tsx correctly!");
}
